...
\citet{GPN}によると, $\mathcal{PN}_3(\bm \mu,\Sigma$)の球形データの場合, 単位球面上の方向を表す$U = (\cos\Theta_1 \sin \Theta_2, \sin\Theta_1 \sin \Theta_2, \cos \Theta_2)^T$における$\bm \theta = (\theta_1, \theta_2)^T$の確率密度を式(\ref{PNC})に示す.
...
情報量基準(WAIC)を用いて, 混合分布のクラスター推定を行う. AICやBICなどの情報量基準には, 事後分布が正規分布で近似されている必要があるなど, 様々な制約が存在するが, WAICは真の分布, 確率モデル, 事前分布がどのような場合でも用いることができる. WAICは式$(\ref{WAIC1})$で求められる.
...
\if0
\begin{wrapfigure}[15]{r}[0mm]{75mm}
\vspace{-0.8cm}
\begin{center}
\includegraphics[clip,height= 70mm]{data/Rplot.png}
\end{center}
 \vspace{-0.9cm}
\caption{クラスタリング結果}
\label{skmeans}
\end{wrapfigure}
\fi
各クラスターにおける$0$を除く平均評価値は$(3.57, 3.65,$ $3.45, 3.60)$となった. t-SNEを用いて正規化を行うことで, データを球面上に偏ることなく配置させることが可能になるが, クラスターごとの特徴が顕著に現れなかった. 同様のデータに対して, 球面上の$k$平均法を用いて検証を行ったが, クラスターごとの特徴は現れなかった.  問題としては, ユーザーの評価において$0$が非常に多い点であり. 次元圧縮において$0$のデータも特徴として扱われてしまうが, 実際には特徴としての意味が存在しないと考えられる. 

%%%%%%%%%%%%%%%%%%%%%%%  section5 まとめ %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{-1zh}
\section{まとめ}

本研究では高次元データを$3$次元に圧縮し, クラスタリングを試みたが, クラスターごとの特徴が顕著に現れなかった. 一般化射影正規分布では任意次元の超球面において分布を生成することができるが, 次元が大きくなると計算量の問題で実装することができなかったので, 今後取り組みたい.
...
Hernandez-Stumpfhauser (2016) \citet{
