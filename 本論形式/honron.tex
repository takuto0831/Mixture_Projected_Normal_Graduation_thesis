...
サンプリング数を$20000$回, バーンイン期間を$10000$回として, MCMCサンプリングによりパラメータを推定した. 得られたパラメータを式(\ref{sim_parameter_pred})に示す. サンプリング数とはリープ$\cdot$フロッグ法により採択するサンプル数$t$を表し, バーンイン期間はパラメータの収束に要する期間であり, 得られたサンプルは各パラメータのサンプルには含まない. 
