...
本研究では, ある美容院の顧客データ, 会計履歴データ, 会計明細データを用いて顧客のクラスタリングを行う. 
...
会計履歴 & 会計ID, 販売店舗ID, 会計日, 会計時刻, 顧客ID, 会計税込売上, 会計消費税,取引種別, \\
             &  POS入力担当者ID, 会計主担当者ID, 会計指名区分, DV商品券金額, DV商品券枚数,  現金, \\
             & クレジット, 電子マネー, 売掛金, 他社商品券金額, 他社商品券枚数, 商品券利用枚数, 利用商品券ID, \\
            & ポイント利用額, ポイント利用区分, ポイント付与額, ポイント残高, 累積来店回数, 曜日 \\
\hline
\end{tabular}
\end{center}
\end{table}
