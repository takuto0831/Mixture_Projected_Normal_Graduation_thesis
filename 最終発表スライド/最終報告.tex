...
\vspace{0.2cm}
\item 
単位超球面上$(\mathbb{S}^{p-1})$など, 幾何的な分布特性を持つデータをユークリッド空間上のデータとして扱うと良い解析が行えない場合がある.
\end{itemize}
...
\begin{itemize}
\item
本研究では, 方向データの分布として知られる, 射影正規分布$(\mathcal{PN}_d)$の混合分布によるクラスタリングの性能評価を行う.
\end{itemize}
%\end{alertblock}
\end{block}
\end{frame}
...
\begin{itemize}
	...
	\item 
	二つの角度$\theta_1, \theta_2$(実線)に対して, 円周上の平均方向は, $\frac{1}{2} (\cos \theta_1 + \cos \theta_2,\sin \theta_1 + \sin \theta_2)$(破線)と定義する.
...
\begin{itemize}
\item 
図\ref{thetasample}における, $x$軸の正の向きと$OS$のなす角を$\theta_1$, $z$軸の正の向きと$OR$のなす角を$\theta_2$と定義する.
\end{itemize}
...
角度データ $\bm \theta$ が得られたときの, パラメータベクトル $\bm \eta$の事後分布を, 事前分布$p(\bm \eta)$を用いて以下で表せる.
...
\noindent
事後分布$p(\bm \eta | \bm \theta)$は尤度$p(\bm \theta | \bm \eta)$と事前分布 $p(\bm \eta)$の積に比例するので, 事後分布の代わりに, $p(\bm \theta | \bm \eta) p(\bm \eta)$ から乱数サンプルを発生させる. この方法で得られたサンプルをMCMCサンプルと呼ぶ. 
\end{frame}
\begin{frame}{混合射影正規分布(3/3)}

\begin{itemize}
	\item 球面上における混合射影正規分布のパラメータベクトル$\bm \eta$の対数事後分布を以下に示す.
\end{itemize}
...
\vspace{0.2cm}
\item
得られたパラメータベクトルとデータから, 各データがどのカテゴリに所属しているかを確率的に表すことができる. 最も高い確率を示す, カテゴリの番号をデータのラベルとすることで, 各データをクラスタリングすることができる.　
%\item
...
\begin{itemize}
\item
$4$つの異なる射影正規分布に従う乱数を混合することで, 混合射影正規分布の人工データ(データ数$n=2000$)を生成する.
...
\vspace{0.2cm}
\item
シミュレーションにおけるパラメータ推定の一例を表\ref{cross1}に示す. 表中の( )内の値は, 95\%信頼区間である.
\end{itemize}
...
%\hline
%  & \multicolumn{4}{c}{真値} & \multicolumn{4}{c}{事後平均}\\ \hline
...
\begin{itemize}
	\item 
	横軸を真のカテゴリ, 縦軸を予測のカテゴリとして混合射影正規分布による分類精度を表\ref{cross2}に示す.
\end{itemize}

\begin{table}[tbp]
\begin{center}
\caption{混合射影正規分布による分類精度}
\label{cross2}
\begin{tabular}{c|c|c c c c}
\hline
 &  & \multicolumn{4}{c}{予測のカテゴリ} \\ \hline
 &  & 1 & 2 & 3 & 4  \\ \hline 
...
\begin{itemize}
\item
各カテゴリのデータが重なる部分を, クラスタリングにより明確に分類されている. 混合分布によるクラスタリングでは, 複数の分布が密集している部分では, 誤差が大きくなると考えられる.
\end{itemize}
...
\noindent
ここで, $n$は$\bm \theta$のデータ数である. また$E_{\bm \eta}[\cdot]$は$\bm \eta$の事後分布の下で評価した期待値である.  
...
\begin{itemize}
	...
	\vspace{0.2cm}
	\item
	$4$つのコンポーネントにおける混合射影正規分布のパラメータを以下に示す. なお, $\hat {\bm w}$は各カテゴリの混合比率を表し, パラメータの添え字は各カテゴリの番号に対応している.
\end{itemize}
...
\end{center}
\caption{カテゴリごとのジャンルへの平均評価値}
\label{clustergenre}
\end{figure}

\begin{itemize}
	\item カテゴリ2はすべてのジャンルへの評価値が高く, その他のカテゴリでは, ジャンルへの評価値に差異が現れた.
\end{itemize}
\end{frame}
...
\item
一般化射影正規分布では任意次元の超球面において分布を生成することができるので, 次元を圧縮することなく超球面上でのクラスタリングに取り組みたい. 
...
\begin{frame}{混合分布によるクラスタリングの図例}

\begin{itemize}
	\item 混合分布によるクラスタリングでは, $\alpha$をカテゴリ1に, $\beta$をカテゴリ2に分類する.
\end{itemize}
...
マルコフ連鎖で表されるダイナミクスを用いて, 与えられた確率分布からのサンプルを得るアルゴリズムをマルコフ連鎖モンテカルロ法(Markov Chain Monte Carlo Methods)と呼ぶ.

\begin{itemize}
	\item 
	サンプリング数 : 発生させる各パラメータのサンプリング数.
	\vspace{0.2cm}
	\item
...
得られたユーザごとジャンルへの評価値から, カテゴリごとのジャンルへの評価値を求める. 図\ref{genre_count}に示すように, 各ジャンルは均等に出現していないので, 各ジャンルの影響を基準化し, 各コンポーネントでジャンルへの評価値の平均値を算出する.
